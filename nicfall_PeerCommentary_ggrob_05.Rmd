---
title: "nicfall_PeerCommentary_ggrob_05"
...
First of all I apologize for submitting this so late. I was attempting to figure out my own code to provide feedback, but have not been able to figure anything out. Therefore, I think your code is way better than mine. I think you do the bootstrapping correctly. My values for standard error and confidence intervals are also way different than the model, so maybe they are supposed to be? Not sure. I like how you remove the NAs from the dataset, I might try that and see if it changes anything. Also good job commentating on your code!
...
1. I don't understand why my average slope & intercept values after bootstrapping are completely off from what the real ones were from the whole dataset (see below) - maybe I made a mistake somewhere. 
...
2. Similarly, my CI from the quantiles of my dataset vs. running this type of calculation: "lower.slope <- mean(set.slope) + qnorm(0.025, mean=0, sd=1)*se(set.slope)" were completely different - but in Chris's example on module 7, they were very similar results. Again maybe I made a mistake somewhere. 
...
4. The use of "the former" & "the latter" was very confusing. 
