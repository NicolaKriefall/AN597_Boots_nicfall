...
```{r}
#Not sure how to calculate the 95% CI for a linear model
```


# Challenges

1. I don't understand why my average slope & intercept values after bootstrapping are completely off from what the real ones were from the whole dataset - maybe I made a mistake somewhere. 
...
4. The use of "the former" & "the latter" was very confusing. Also we have two standard errors right? One for slope & one for intercept? So am I comparing all three in the section about 'the former' & 'the latter'?

5. Couldn't figure out how to calculate the 95% CI for the full dataset. I think my issue overall is that the terminology is the same within a linear model as for a sampling distribution & I can't keep track of what we're referring to anymore. 